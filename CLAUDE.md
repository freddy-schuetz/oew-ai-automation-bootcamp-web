...
7. **Antwortet `/deploy` mit `409` und einem `link`:** Dann fehlt der Deploy-Key im Repository (kommt vor, wenn beim Anlegen des Repos etwas schiefging). Gib der Person genau diesen Link und sag: einmal öffnen, bei GitHub auf **„Authorize"** klicken. Der Hub trägt den Schlüssel dann selbst ein, nur mit Leserecht. Danach denselben `/deploy`-Aufruf wiederholen — fertig. Den Link kannst du auch gezielt holen: `curl -s -X POST https://hub-oew.buildbar.at/deploykey -d "token=<TOKEN>"`.
8. **Bleibt die Adresse nach ein paar Minuten stumm** (503, „no available server"): `curl -s -X POST https://hub-oew.buildbar.at/status -d "token=<TOKEN>"` nennt Ausgang, Commit und die Fehlerzeilen des letzten Builds. Sag der Person, was dort steht, statt ein Serverproblem zu vermuten.
